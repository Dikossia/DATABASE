...
CREATE TABLESPACE course_data
  LOCATION 'C:/pgsql/tablespaces/courses';

ALTER TABLESPACE course_data OWNER TO CURRENT_USER;


--CREATE DATABASE university_distributed    КОДИРОВКА НЕ СООТВЕТСТВОВАЛ, пришлось менять
...
CREATE TABLE students (
    student_id     SERIAL PRIMARY KEY,          -- автоинкремент int
    ...
    gpa            NUMERIC(4,2),                -- десятичное, 2 знака после запятой
    is_active      BOOLEAN,
...
CREATE TABLE professors (
    ...
    hire_date        DATE,
    salary           NUMERIC(14,2),             -- "large decimal"
    is_tenured       BOOLEAN,
...
CREATE TABLE courses (
    ...
    is_online      BOOLEAN,
    created_at     TIMESTAMP WITHOUT TIME ZONE  -- без часового пояса
);
...
--Task 5.2
-- DROP DATABASE IF EXISTS university_test; НЕ ПОЛУЧИЛОСЬ ПОТОМУ ЧТО ИС-ТЕМПЛЕЙТ ТРУ
UPDATE pg_database
